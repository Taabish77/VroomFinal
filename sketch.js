...
var carImage,OppImg,Obs1Image,Obs1;
  ...
  if (gameState===PLAY){
   ...
   Opponent();
    ...
    if(obstaclesGroup.isTouching(car1)){
        collidedSound.play()
        RacerGroup.setVelocityXEach(17);
        gameState = END;
    }
  }
...
function spawnObstacles() {
  if(frameCount % 100 === 0) {
    ...
    obstacle.velocityX = -(6 + 3*score/20);
...
function Opponent() {
    ...
    var racer = createSprite(200,height-30,40,10);
    racer.addImage(OppImg);
    racer.scale = 0.1;
    ///cloud.velocityX = -3;
    if(racer.isTouching(car1)){
      textSize(50);
      fill("red");
